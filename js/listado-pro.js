...
let editar =document.querySelector('#btn-edit')
let rol = document.querySelector("#rol-usuario");
...
function buscar() {
    ...
    let palabraBuscada = buscar.toLowerCase();
    let resultado = products.filter(x => x.nombre.toLowerCase().includes(palabraBuscada))
    
     if (resultado.length > 0) {
        resultado.forEach((dato, i)=>{
...
window.editDataTable = function ( pos ){
    let products = [];
    let productsSave = JSON.parse(localStorage.getItem("datosTabla"));
    if (productsSave != null) {
        products = productsSave;
...
window.deleteDataTable = function ( pos ){
    let products = [];
    let productsSave = JSON.parse(localStorage.getItem("datosTabla"));
    if (productsSave != null) {
        products = productsSave;
